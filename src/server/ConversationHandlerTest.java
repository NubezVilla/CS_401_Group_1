...
public class ConversationHandlerTest {
    ...
    private ClientHandler dummyClient;
    private TestClientHandler testDummyClient;
    ...
    public void setUp() throws IOException {
        ...
        testDummyClient = new TestClientHandler();
        conversationHandler = new ConversationHandler(out, in, testDummyClient);
        ...
        UserData.getInstance().addUser(testUserToModify);
        UserData.getInstance().addUser(testHostUser);
        UserData.getInstance().addConversation(activeConversation);
    ...
    public void testHandleGetConversation_NotFound_Fails() {
        ...
        assertDoesNotThrow(() -> {
            conversationHandler.handleGetConversation(badRequest, activeConversation.getID());
    ...
    public void testHandleGetConversation_Success() throws IOException {
        ...
        //send the right data
        Wrapper clientAck = new Wrapper(new Message("GOT IT", "Client"), ResponseType.SENDING_MESSAGE);
        ...
        ConversationHandler customHandler = new ConversationHandler(out, loadedMockIn, testDummyClient);

        //send the request
        Wrapper request = new Wrapper(activeConversation.getID(), RequestType.GET_CONVERSATION);

        assertDoesNotThrow(() -> {
            customHandler.handleGetConversation(request, activeConversation.getID());
    ...
    public void testHandleCreateConversation_Success() {
    	//i need to make a fake conversation with users
        User currentUser = new User("current", "pass");
        User otherUser = new User("other", "pass");

        UserData.getInstance().addUser(currentUser);
        UserData.getInstance().addUser(otherUser);
        ...
        assertAll(
            () -> assertEquals(1, currentUser.getConversations().size()),
            () -> assertEquals(1, otherUser.getConversations().size()),
            () -> assertEquals(
                    currentUser.getConversations(),
                    otherUser.getConversations()
            ),
            () -> assertEquals(
                    ResponseType.CREATE_CONVERSATION_SUCCESS,
                    testDummyClient.getLastSent().getResponseType()
            )
        );
    }
    @Test
    public void testHandleCreateGroupConversation_Success() {
        ...
        UserData.getInstance().addUser(creator);
        UserData.getInstance().addUser(member);
        ...
        Wrapper request = new Wrapper(base, RequestType.CREATE_GROUP_CONVERSATION);

        conversationHandler.handleCreateGroupConversation(out, request, creator.getUserID());

        assertAll(
                () -> assertEquals(1, creator.getConversations().size()),
                () -> assertEquals(1, member.getConversations().size()),
                () -> assertEquals(
                        creator.getConversations(),
                        member.getConversations()
                ),
                () -> assertEquals(
                        ResponseType.GROUP_CREATION_SUCCESS,
                        testDummyClient.getLastSent().getResponseType()
                )
            );
    }
    ...
    //i need to simulate a client that receives a message
    //this is because sendToClient is private and I need access to it
    //i can create a class that extends clientHandler and make a sendToClient method for testing
    private static class TestClientHandler extends ClientHandler {
        private Wrapper lastSent;
        ...
        @Override
        public synchronized void sendToClient(Wrapper obj) {
            lastSent = obj;
        }

        public Wrapper getLastSent() {
            return lastSent;
        }
    }
}
