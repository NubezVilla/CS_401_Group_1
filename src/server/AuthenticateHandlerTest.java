...
public class AuthenticateHandlerTest {
    ...
    public void setUp() throws IOException {
        ...
        ObjectOutputStream dummyOut = new ObjectOutputStream(dummyOutput);
        dummyOut.flush();

        //get the input stream
        ByteArrayInputStream dummyInputStream = new ByteArrayInputStream(dummyOutput.toByteArray());
        in = new ObjectInputStream(dummyInputStream);
        ...
        UserData.getInstance().addUser(validTestUser);
    ...
    /*** TESTING LOGGING IN ***/
    @Test
    public void testHandleLogin_Success() throws IOException {
    	//i have to fake an input stream to test this method
    	ByteArrayOutputStream prepByteStream = new ByteArrayOutputStream();
    	ObjectOutputStream prepOutputStream = new ObjectOutputStream(prepByteStream);
    	
    	//responseHandler expects a ResponseType
    	Wrapper client = new Wrapper (new Message("Message recieved", "Client"), ResponseType.LOGIN_SUCCESS);
    	prepOutputStream.writeObject(client);
    	prepOutputStream.flush();
    	
    	//make the input stream
    	ByteArrayInputStream bais = new ByteArrayInputStream(prepByteStream.toByteArray());
        ObjectInputStream loadedInput = new ObjectInputStream(bais);
        
        //create a AuthenticateHandler object with the new input and output
        AuthenticateHandler customAuthHandle = new AuthenticateHandler(out, loadedInput);
        
    	
        //make a login request
        Wrapper loginRequest = new Wrapper(validCredentials, RequestType.LOGIN);

        boolean result = customAuthHandle.handleLogin(loginRequest);

        //check that the credentials are correct
        //check that the account exists
        //check that the user is logged in
        assertTrue(result, "Handler should return true for valid credentials.");
        assertNotNull(customAuthHandle.getUserAccount(), "User account should be populated.");
        assertTrue(customAuthHandle.isLoggedIn(), "Internal loggedIn state should be true.");
    }
    ...
    public void testHandleLogout_Success() throws IOException {
        ...
        Wrapper secondaryPayload = new Wrapper(fakeConversations, ResponseType.SENDING_CONVERSATIONS);
    ...
    public void testHandleRegister_Success_ITUser() throws Exception {
        ...
        itUser.setIT(true);
        UserData.getInstance().addUser(itUser);
        ...
        newUserInfo.add("newuser");
        newUserInfo.add("newpass");
        newUserInfo.add("New User");
        newUserInfo.add("Developer");

        Wrapper request = new Wrapper(newUserInfo, RequestType.REGISTER);

        handler.handleRegister(out, request, true);

        LoginInfo expectedLogin = new LoginInfo("newuser", "newpass");
        User createdUser = UserData.getInstance().getUserByLoginHash(expectedLogin.hashCode());

        assertNotNull(createdUser, "The new user should be successfully added to UserData.");
        assertTrue(createdUser.isIT(), "The new user should be marked as an IT user.");
    }
    @Test
    public void testHandleRegister_DuplicateUser_Fails() throws Exception {
        ...
        UserData.getInstance().addUser(existingUser);
        ...
        ArrayList<String> duplicateUserInfo = new ArrayList<>();
        duplicateUserInfo.add("sameuser");
        duplicateUserInfo.add("samepass");
        duplicateUserInfo.add("Duplicate User");
        duplicateUserInfo.add("Tester");

        Wrapper request = new Wrapper(duplicateUserInfo, RequestType.REGISTER);

        handler.handleRegister(out, request, true);

        User foundUser = UserData.getInstance()
                .getUserByLoginHash(existingUser.getLoginInfo().hashCode());

        assertSame(existingUser, foundUser, "Duplicate user should not replace the existing user.");
    }
    @Test
